...
# Windows: $env:ProgramData\Bookworm
# Linux:   $env:HOME/Bookworm  (PSU runs under the service user; writable)
if ($IsWindows) {
    $BasePath = Join-Path $env:ProgramData 'Bookworm'
}
elseif ($IsLinux) {
    if (-not $env:HOME) {
        throw "Linux detected but `$env:HOME is not set. Cannot determine Bookworm data folder."
    }
    $BasePath = Join-Path $env:HOME 'Bookworm'
}
else {
    # Fallback for other platforms
    $BasePath = Join-Path ([System.IO.Path]::GetTempPath()) 'Bookworm'
}

# Define stable database path
$script:DatabasePath = Join-Path $BasePath 'Bookworm.db'
